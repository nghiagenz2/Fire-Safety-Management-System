...
function canonicalFloorIdFromValue(floor = '') {
  ...
  if (normalized.includes('tret')) {
    return 'floor_tret';
  }

  // Use \d+ (greedy) with a word-boundary alternative to always capture full number
  const match = normalized.match(/(?:floor|tang|tang)?(\d+)/);
  if (match) {
    return `floor_${parseInt(match[1], 10)}`;
  }
...
function labelFromCanonicalFloorId(floorId = '') {
  ...
  if (match) {
    return `Tầng ${parseInt(match[1], 10)}`;
  }
...
async function fallbackList(filters = {}) {
  ...
  return escapes
    ...
    .filter((escape) => {
      if (filters.status && filters.status !== 'all' && escape.status !== filters.status) {
        return false;
      }

      if (filters.floor && filters.floor !== 'all' && escape.floor !== filters.floor) {
        return false;
      }
